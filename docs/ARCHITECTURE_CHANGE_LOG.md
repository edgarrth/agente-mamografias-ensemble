...
## v0.36.0 — perfil productivo CPU preempaquetado

- Añade `deployment/production/docker-compose.prod.yml` sin instrucciones `build:`: el VPS consume imágenes preconstruidas.
- Añade Caddy como único punto público (80/443) con HTTPS automático cuando se configura un dominio y Basic Auth.
- Reemplaza el bind mount de `HOST_WORKSPACE` únicamente en el perfil productivo por `mammography-runtime-workspace`.
- Añade una imagen `mammography-runtime-assets` que siembra el metarepository ya resuelto y `workspace/models` sin datasets ni outputs Batch.
- Añade scripts separados para publicar imágenes de plataforma/modelos, descargar/retaggear las seis imágenes requeridas y desplegar en VPS.
- El perfil fuerza CPU a nivel de variables del deployment y mantiene `ALLOW_GPU=false`.
- No cambia la lógica de `single_case.py`, `pipeline.py`, `orientation_policy.py`, `soft_voting.py`, `api.py` ni `ui/streamlit_app.py`.
- `model_runner/api.py` conserva la lógica y cambia únicamente su identificador de versión a 0.36.0.
